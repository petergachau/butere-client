...
export const createRevision = createAsyncThunk(
  ...
  async (values) => {
    try {
      const response = await api.createRevision(values)
...
export const getRelatedTours = createAsyncThunk(
  ...
  const revisionSlice = createSlice({
    ...
    extraReducers: {
      [createRevision.pending]: (state, action) => {
        state.loading = true;
      },
      [createRevision.fulfilled]: (state, action) => {
        state.loading = false;
        state.projects = action.payload;
      },
      [createRevision.rejected]: (state, action) => {
        state.loading = false;
